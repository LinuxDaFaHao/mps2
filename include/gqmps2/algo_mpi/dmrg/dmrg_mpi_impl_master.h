...
#ifndef GQMPS2_ALGO_MPI_DMRG_DMRG_MPI_IMPL_MASTER_H
#define GQMPS2_ALGO_MPI_DMRG_DMRG_MPI_IMPL_MASTER_H
...
class DMRGMPIMasterExecutor : public Executor {
 ...
 private:
  ...
  RightBlockOperatorGroup<Tensor> InitUpdateRightBlockOps_(
      const RightBlockOperatorGroup<GQTensor<TenElemT, QNT>> &rog,
      const GQTensor<TenElemT, QNT> &mps_ten,               // site i
  ...
  Tensor *StaticHamiltonianMultiplyState_(Tensor &state, GQTEN_Double &overlap);
...
template<typename TenElemT, typename QNT>
RightBlockOperatorGroup<GQTensor<TenElemT, QNT>> DMRGMPIMasterExecutor<TenElemT, QNT>::InitUpdateRightBlockOps_(
    const RightBlockOperatorGroup<GQTensor<TenElemT, QNT>> &rog,
    const GQTensor<TenElemT, QNT> &mps_ten,               // site i
    const SparMat<GQTensor<TenElemT, QNT>> &mat_repr_mpo   // site i
) {
...
#endif
  SendBroadCastGQTensor(world_, mps_ten, kMasterRank);
#ifdef GQMPS2_MPI_TIMING_MODE
  ...
  if (task_num <= slave_num_) {
    ...
    for (size_t i = 0; i < task_num; i++) {
      auto recv_status = recv_gqten(world_, i + 1, i, rog_next[i]);
    }
  } else {
    ...
    for (size_t task_id = slave_num_; task_id < task_num; task_id++) {
      Tensor res;
      auto recv_status = recv_gqten(world_, mpi::any_source, mpi::any_tag, res);
...
void DMRGMPIMasterExecutor<TenElemT, QNT>::SendBlockSiteHamiltonianTermGroup_(
  ...
  for (size_t i = 0; i < num_terms; i++) {
    send_gqten(world_, id, i * id, *block_site_terms[i][0]);
    send_gqten(world_, id, i * id, *block_site_terms[i][1]);
  }
}
...
void DMRGMPIMasterExecutor<TenElemT, QNT>::GrowLeftBlockOps_() {
  SendBroadCastGQTensor(world_, mps_[l_site_], kMasterRank);
...
void DMRGMPIMasterExecutor<TenElemT, QNT>::GrowRightBlockOps_() {
  SendBroadCastGQTensor(world_, mps_[r_site_], kMasterRank);
