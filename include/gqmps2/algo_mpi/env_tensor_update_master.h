...
*
* Description: GraceQ/MPS2 project. Environment tensors update function, master side.
*/
...
#ifndef GQMPS2_ALGO_MPI_ENV_TENSOR_UPDATE_MASTER_H
#define GQMPS2_ALGO_MPI_ENV_TENSOR_UPDATE_MASTER_H
...
template<typename TenElemT, typename QNT>
inline GQTensor<TenElemT, QNT> *MasterGrowLeftEnvironmentInit(
    const GQTensor<TenElemT, QNT> &lenv,
    const GQTensor<TenElemT, QNT> &mpo,
    const GQTensor<TenElemT, QNT> &mps,
    mpi::communicator &world
) {
  SendBroadCastGQTensor(world, mpo, kMasterRank);
  SendBroadCastGQTensor(world, lenv, kMasterRank);
  return MasterGrowLeftEnvironment(lenv, mpo, mps, world);
}

template<typename TenElemT, typename QNT>
inline GQTensor<TenElemT, QNT> *MasterGrowRightEnvironmentInit(
    const GQTensor<TenElemT, QNT> &renv,
    const GQTensor<TenElemT, QNT> &mpo,
    const GQTensor<TenElemT, QNT> &mps,
    mpi::communicator &world
...
#endif //GRACEQ_MPS2_INCLUDE_GQMPS2_ALGO_MPI_ENV_TENSOR_UPDATE_MASTER_H
