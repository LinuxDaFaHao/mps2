...
TEST_F(TestNoisedTwoSiteAlgorithmSpinSystem, 1DIsing) {
  ...
  RunTestTwoSiteNoiseAlgorithmCase(dmps, dmpo, sweep_params, -0.25 * (N - 1), 1.0E-10);
  ...
  RunTestTwoSiteNoiseAlgorithmCase(zmps, zmpo, sweep_params, -0.25 * (N - 1), 1.0E-10);
...
TEST_F(TestTwoSiteAlgorithmHubbardSystem, 2Dcase) {
  ...
  auto sweep_params = FiniteVMPSSweepParams(
      ...
      16, 16, 1.0E-9,
      LanczosParams(1.0E-8, 20),
      std::vector<double>{0.1, 0.01, 0.001}
  );
...
TEST_F(TestTwoSiteAlgorithmElectronPhononSystem, holsteinchain) {
  ...
  auto sweep_params = FiniteVMPSSweepParams(
      ...
      std::vector<double>{0.2, 0.1, 0.1, 0.1, 0.01, 0.01, 0.001, 0.0001, 0.00001, 0.0}
  );
  dmps.Dump(sweep_params.mps_path, true);
